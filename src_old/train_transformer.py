...
def run_experiment(args, seed):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if mse < best_mse:
            ...
            torch.save(
                ...
                os.path.join(
                    args.save_dir,
                    f"transformer_output{args.output_len}_seed{seed}.pth"
                )
            )
    ...
    plot_prediction(
        ...
        save_path=os.path.join(
            args.figure_dir,
            f"transformer_output{args.output_len}_seed{seed}.png"
        ),
        title=f"Transformer Prediction vs Ground Truth, Output={args.output_len}, Seed={seed}"
    )
