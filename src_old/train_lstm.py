...
def run_experiment(args, seed):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if mse < best_mse:
            ...
            torch.save(
                ...
                os.path.join(
                    args.save_dir,
                    f"lstm_output{args.output_len}_seed{seed}.pth"
                )
            )
    ...
    _, final_mse, final_mae, preds, trues = evaluate(
        model,
        test_loader,
        criterion,
        device,
    ...
    plot_prediction(
        ...
        save_path=os.path.join(
            args.figure_dir,
            f"lstm_output{args.output_len}_seed{seed}.png"
        ),
        title=f"LSTM Prediction vs Ground Truth, Output={args.output_len}, Seed={seed}"
    )
